...
* 위 `MortgageAccessor` 클래스의 목적은 신용 등급이 나쁜 경우 대출 신청을 거부하기 위함이다.
...
* `MortgageAccessor` 클래스를 `MortgageAccessor` 와 `CreditRatingChecker` 로 분할하고, `MortgageAccessor` 가 `CreditRatingChecker` 에 의존하게 코드를 작성할 수 있다.
